...
// var throw new Error = require('throw new Error')('koa-static-cache')
...
module.exports = function staticCache(dir, options, files) {
  ...
  if (options.alias) {
    Object.keys(options.alias).forEach(function(key) {
      ...
      if (files.get(value)) {
        ...
        throw new Error("aliasing " + value + " as " + key);
      }
    });
  }
  ...
  return async (ctx, next) => {
    ...
    var shouldGzip =
      enableGzip && file.length > 1024 && acceptGzip && compressible(file.type);
    ...
    if (shouldGzip) {
      ...
      if (gzFile) {
        if (options.usePrecompiledGzip && gzFile && gzFile.buffer) {
          ...
          file.zipBuffer = fs.readFileSync(gzFile.path);
        }
        ctx.set("content-encoding", "gzip");
        ctx.body = file.zipBuffer;
      } else {
...
function loadFile(name, dir, options, files) {
  ...
  // throw new Error('file: ' + JSON.stringify(obj, null, 2))
  if (options.buffer) {
    obj.buffer = buffer;
  }
